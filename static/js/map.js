...
*/    
   ...
   function centerAlign(){
      // align center
      // have to calculate if there are bigger cases
      // after delete zoom is not correct!
      /*
      for(var i = 0; i<levelNum; i++){
         var space = 0;
         for(var j=0; j<levelCanvasNum[i]; j++){
            space += parseInt($("#level-" + i + "-" + j + ".canvasBox").css("zoom")*100);
            console.log(i,j,$("#level-" + i + "-" + j + ".canvasBox").css("zoom")*100);
         }
         console.log(i,space);
         /*
         var space = 100 - levelHeight[i]*levelCanvasNum[i];
         if(space){
            for(var j=0; j<levelCanvasNum[i]; j++)
               $("#level-" + i + "-" + j + ".canvasBox").css("left", (space/2) + "%");
         }
         else{
            for(var j=0; j<levelCanvasNum[i]; j++)
               $("#level-" + i + "-" + j + ".canvasBox").css("left", "");
         }
         *//*
      }
      */
